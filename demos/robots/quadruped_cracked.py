...
def send_to_simulator(sim):
    ...
    # Stuur sinusvormige inputs naar motorneuronen
    for idx, joint in enumerate(motor_neurons):
        # Genereer random parameters
        phase_offset = np.random.uniform(0, 2 * math.pi)     # willekeurige fase tussen 0 en 2π
        frequency = np.random.uniform(0.5, 2.0)              # frequentie tussen 0.5 en 2 Hz
        amplitude = np.random.uniform(0.2, 1.0)              # amplitude tussen 0.2 en 1.0 rad
